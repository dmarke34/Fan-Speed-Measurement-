...
{ 
    ...
    TRISB = 0x01;                   // Set PORTB bit 0 as input; otherwise output
    TRISC = 0x01;                   // Set PORTC bit 0 as input; otherwise output
...
void main() 
{
    ...
    while (1)
    {
        ...
        if (nec_ok == 1)
        {
            ...
            for (int j=0; j< 21; j++)
            {
                if (Nec_code1 == array1[j])                // Set found to element location of the buttons
                {
                    found = j;
                    break;
                }
            ...
            {
                Do_Beep();                                  // Initialize beep if button is listed in array
                printf ("button = %d \r\n", found);

                if (found == 5) {                           // Turn on/off fan if 'Play/Pause' is pressed
                    Toggle_Fan();
                }
                else if (found == 6) {                      // Decrease fan speed if '-' is pressed
                    Decrease_Speed();
                }
                else if (found == 7) {                      // Increase fan speed if '+' is pressed
                    Increase_Speed();
                }
                else if (found == 8)                             // Check if button pressed is EQ
                {
